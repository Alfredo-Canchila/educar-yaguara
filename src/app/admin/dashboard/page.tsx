...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      const { data, error } = await supabase.from("leads").select("estado, created_at");
      if (data) {
        ...
        setStats({
          ...
          contactadosHoy: data.filter(l => l.estado === "Contactado" && l.created_at.startsWith(today)).length,
        });
      }
    };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-[var(--color-educar-dark)]">Dashboard General</h1>
        <p className="text-gray-500 mt-2">Gestiona las inscripciones y prospectos de Educar Yaguará.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500">Mensajes Sin Leer (Nuevos)</p>
